...
### New: User Auth + Sessions
- Added `User` authentication endpoints: `POST /api/users`, `POST /api/login`, `GET /api/me`.
- Session persistence for users via `Session` model.
- JWT generator extended for users.
- See `backend/docs/DEV_GUIDE.md` for environment setup, endpoints, and smoke test instructions.
